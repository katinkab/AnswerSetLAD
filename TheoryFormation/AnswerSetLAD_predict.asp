...
%Input: Testset of observations, primecover (including theorypat(N,S,D,(V,Q)). theorycov(N,S,D,(Z,Y)). train_nbrnegobs(T). train_nbrposobs(P).)
...
testset_not_allright(N,S,D,(Z,X)) :- i(Z,X,_,_), theorypat(N,S,D,(V,Q)) , not i(Z,X,V,Q).
testset_allright(N,S,D,(Z,X)) :- not testset_not_allright(N,S,D,(Z,X)), i(Z,X,_,_), theorypat(N,S,D,(_,_)).

%coverage of theorypatterns
coverage(N,S,D,C) :- theorypat(N,S,D,_), C = #sum{1, (Z,X) : theorycov(N,S,D,(Z,X))}.

%prediction
%theorypatterns are weighted by their coverage normalized by the total number of positive (negative) observations
%predict positive:
