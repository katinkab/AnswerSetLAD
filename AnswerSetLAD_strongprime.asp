...
% ------------------------------------------------ %
%                                                  %  
%        STRONG PRIME PATTERN GENERATION           % 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
1{pat(S,B):lit(S,B)}.
%%%wir arbeiten ohne degree, also muss ich den hier zählen um ihn später zu nutzen
countdegree(D):-D=#sum{1,(S,B):pat(S,B)}.
#show countdegree/1.

%%%START:AUS PRIME PATTERN!
...
%%%for prime pattern calculation: coverage for every obs X (of sign W) by pat WITHOUT literal (S,B)
del_covered(W,X,(S,B)):-i(W,X,_,_),pat(S,B),not i(W,X,S,B),#sum{1,(T,C):pat(T,C),i(W,X,T,C),(T,C)!=(S,B)}=D-1, countdegree(D).
%%%homogeneity of pat WITHOUT literal (S,B) still good
homgood(S,B):-pat(S,B),#sum{homogeneity-100,X:del_covered(W,X,(S,B)),W=sign;homogeneity,X:del_covered(W,X,(S,B)),W!=sign}<=0.

%%%TEST%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%is homogeneity of shorter pattern (WITHOUT literal (S,B)) still good? -> then pat NOT PRIME
:-homgood(S,B).
%%%prime end
